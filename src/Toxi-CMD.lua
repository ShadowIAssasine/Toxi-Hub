...
--[[ Brand ]]--
local mainName='Toxi CMD'
local testingName='TCMD Testing'
local adminName='Toxi'
...
wait();--added wait due to the Http being a bit delayed on returning (should fix the issue where Toxi CMD wouldn't load sometimes)
...
cmd.add({"updatelog","updlog","updates"},{"updatelog (updlog,updates)","show the update logs for Toxi CMD"},function()
	--loadstring(game:HttpGet("https://raw.githubusercontent.com/ltseverydayyou/uuuuuuu/main/Game%20Universe%20Viewer"))()
	gui.updateLogs()
end)
...
cmd.add({"admin"},{"admin","whitelist someone to allow them to use commands"},function(...)
	...
	if Player~=nil and not Admin[Player.UserId] then
		...
		ChatMessage("[Toxi CMD] You've got admin. Prefix: ';'",Player.Name)
		wait(0.2)
		ChatMessage("[Toxi CMD Commands] glue,unglue,fling,fling2,spinfling,unspinfling,fcd,fti,fpp,fireremotes,holdhat",Player.Name)
...
cmd.add({"antichatlogger","acl"},{"antichatlogger (acl)","Anti chat logger"},function()
	...
	TextLabel_2.Text="You are executing an anti-chat-log script meaning that Toxi CMD wouldnt be able to detect when you have chatted meaning if you are on mobile and use the chat to execute commands it wont work. Are you sure you want to execute this?"
...
ImageButton.Image="rbxassetid://9794138570"
